...
Looting the SQLite DB revealed Martin’s MD5 password, which cracked to `nafeelswordsmaster`. 
...
Lines 76-91 shows the function used to run the Python code.  As expected, it contains a blacklist of words that cannot be used.
...
I copy `task.json` to `task2.json` and change the directory to be backed up to `/home/....//root`.  
...
- **Principle of least privilege:** 
	The backup script didn’t need full root reach. A dedicated service account with constrained ACLs would have contained the blast radius.
