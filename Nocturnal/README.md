...
Lines 211-232 shows what happens when a backup is performed.  Line 221 shows that the password is simply concatenated to the `zip` command and that command is simply executed on line 229.
...
Lines 44-54 shows the `cleanEntry()` function with a blacklist to reject common command injection characters.
...
I run a backup and capture the request with BURP.  BURP repeater is then used to send the URL encoded command injection payload to the server in the `password` parameter.
